...
RUN git clone https://github.com/blt/lading.git
RUN cd lading && git checkout rig && cargo build --release && cp target/release/lading /usr/local/bin/lading
...
RUN ["/usr/local/bin/lading", "--version"]
